...
import currencyservice_manager_pb2
import currencyservice_manager_pb2_grpc
import subroutine
...
class CurrencyserviceManagerServicer(currencyservice_manager_pb2_grpc.CurrencyserviceManagerServicer):
    def ExtractMicroserviceData(self, request, context):
        ...
        currencyservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica, cpu_percentage]
        ...
        return currencyservice_manager_pb2.MicroserviceData(
            microservice_name = currencyservice_data[0],
            scaling_action = currencyservice_data[1],
            desired_replicas = currencyservice_data[2],
            current_replicas = currencyservice_data[3],
            cpu_request = currencyservice_data[4],
            max_replicas = currencyservice_data[5],
            cpu_percentage = currencyservice_data[6]
        )

def serve(port):
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    currencyservice_manager_pb2_grpc.add_CurrencyserviceManagerServicer_to_server(CurrencyserviceManagerServicer(), server)
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Currencyservice Manager server started, listening on port " + port)
...
def serve_health(port):
    ...
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Heartbeat server for  Currencyservice Manager started, listening on port " + port)
    server.wait_for_termination()
