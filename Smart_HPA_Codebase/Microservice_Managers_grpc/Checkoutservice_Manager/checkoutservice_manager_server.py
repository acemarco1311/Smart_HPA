...
import checkoutservice_manager_pb2
import checkoutservice_manager_pb2_grpc
import subroutine
...
class CheckoutserviceManagerServicer(checkoutservice_manager_pb2_grpc.CheckoutserviceManagerServicer):
    def ExtractMicroserviceData(self, request, context):
        ...
        checkoutservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica, cpu_percentage]
        ...
        return checkoutservice_manager_pb2.MicroserviceData(
            microservice_name = checkoutservice_data[0],
            scaling_action = checkoutservice_data[1],
            desired_replicas = checkoutservice_data[2],
            current_replicas = checkoutservice_data[3],
            cpu_request = checkoutservice_data[4],
            max_replicas = checkoutservice_data[5],
            cpu_percentage = checkoutservice_data[6]
        )

def serve(port):
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    checkoutservice_manager_pb2_grpc.add_CheckoutserviceManagerServicer_to_server(CheckoutserviceManagerServicer(), server)
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Checkoutservice Manager server started, listening on port " + port)
...
def serve_health(port):
    ...
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Heartbeat server for Checkoutservice Manager started, listening on port " + port)
    server.wait_for_termination()
