...
import frontend_manager_pb2
import frontend_manager_pb2_grpc
import subroutine
...
class FrontendManagerServicer(frontend_manager_pb2_grpc.FrontendManagerServicer):
    def ExtractMicroserviceData(self, request, context):
        ...
        frontend_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica, cpu_percentage]

        # example data
        # frontend_data = ["frontend", "no scale", 1, 1, 50, 5]
        # if error
        # frontend_data = ["frontend", "ERROR", 0, 0, 0, 0]

        return frontend_manager_pb2.MicroserviceData(
            microservice_name = frontend_data[0],
            scaling_action = frontend_data[1],
            desired_replicas = frontend_data[2],
            current_replicas = frontend_data[3],
            cpu_request = frontend_data[4],
            max_replicas = frontend_data[5],
            cpu_percentage = frontend_data[6]
        )

def serve(port):
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    frontend_manager_pb2_grpc.add_FrontendManagerServicer_to_server(FrontendManagerServicer(), server)
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("frontend Manager server started, listening on port " + port)
...
def serve_health(port):
    ...
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Heartbeat server for frontend Manager started, listening on port " + port)
    server.wait_for_termination()
