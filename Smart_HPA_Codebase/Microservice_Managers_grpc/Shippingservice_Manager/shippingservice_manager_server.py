...
import shippingservice_manager_pb2
import shippingservice_manager_pb2_grpc
import subroutine
...
class ShippingserviceManagerServicer(shippingservice_manager_pb2_grpc.ShippingserviceManagerServicer):
    def ExtractMicroserviceData(self, request, context):
        ...
        shippingservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica, cpu_percentage]

        # example data
        # shippingservice_data = ["shippingservice", "no scale", 1, 1, 50, 5]
        # if error
        # shippingservice_data = ["shippingservice", "ERROR", 0, 0, 0, 0]

        return shippingservice_manager_pb2.MicroserviceData(
            microservice_name = shippingservice_data[0],
            scaling_action = shippingservice_data[1],
            desired_replicas = shippingservice_data[2],
            current_replicas = shippingservice_data[3],
            cpu_request = shippingservice_data[4],
            max_replicas = shippingservice_data[5],
            cpu_percentage = shippingservice_data[6]
        )

def serve(port):
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    shippingservice_manager_pb2_grpc.add_ShippingserviceManagerServicer_to_server(ShippingserviceManagerServicer(), server)
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("shippingservice Manager server started, listening on port " + port)
...
def serve_health(port):
    ...
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Heartbeat server for shippingservice Manager started, listening on port " + port)
    server.wait_for_termination()
