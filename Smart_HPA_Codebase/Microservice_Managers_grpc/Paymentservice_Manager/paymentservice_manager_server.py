...
import paymentservice_manager_pb2
import paymentservice_manager_pb2_grpc
import subroutine
...
class PaymentserviceManagerServicer(paymentservice_manager_pb2_grpc.PaymentserviceManagerServicer):
    def ExtractMicroserviceData(self, request, context):
        ...
        paymentservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica, cpu_percentage]

        # example data
        # paymentservice_data = ["paymentservice", "no scale", 1, 1, 50, 5]
        # if error
        # paymentservice_data = ["paymentservice", "ERROR", 0, 0, 0, 0]

        return paymentservice_manager_pb2.MicroserviceData(
            microservice_name = paymentservice_data[0],
            scaling_action = paymentservice_data[1],
            desired_replicas = paymentservice_data[2],
            current_replicas = paymentservice_data[3],
            cpu_request = paymentservice_data[4],
            max_replicas = paymentservice_data[5],
            cpu_percentage = paymentservice_data[6]
        )

def serve(port):
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    paymentservice_manager_pb2_grpc.add_PaymentserviceManagerServicer_to_server(PaymentserviceManagerServicer(), server)
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("paymentservice Manager server started, listening on port " + port)
...
def serve_health(port):
    ...
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Heartbeat server for paymentservice Manager started, listening on port " + port)
    server.wait_for_termination()
