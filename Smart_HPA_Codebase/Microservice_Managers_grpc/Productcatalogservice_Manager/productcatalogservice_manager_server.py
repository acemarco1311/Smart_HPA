...
import productcatalogservice_manager_pb2
import productcatalogservice_manager_pb2_grpc
import subroutine
...
class ProductcatalogserviceManagerServicer(productcatalogservice_manager_pb2_grpc.ProductcatalogserviceManagerServicer):
    def ExtractMicroserviceData(self, request, context):
        ...
        productcatalogservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica, cpu_percentage]

        # example data
        # productcatalogservice_data = ["productcatalogservice", "no scale", 1, 1, 50, 5]
        # if error
        # productcatalogservice_data = ["productcatalogservice", "ERROR", 0, 0, 0, 0]

        return productcatalogservice_manager_pb2.MicroserviceData(
            microservice_name = productcatalogservice_data[0],
            scaling_action = productcatalogservice_data[1],
            desired_replicas = productcatalogservice_data[2],
            current_replicas = productcatalogservice_data[3],
            cpu_request = productcatalogservice_data[4],
            max_replicas = productcatalogservice_data[5],
            cpu_percentage = productcatalogservice_data[6]
        )

def serve(port):
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    productcatalogservice_manager_pb2_grpc.add_ProductcatalogserviceManagerServicer_to_server(ProductcatalogserviceManagerServicer(), server)
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("productcatalogservice Manager server started, listening on port " + port)
...
def serve_health(port):
    ...
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Heartbeat server for productcatalogservice Manager started, listening on port " + port)
    server.wait_for_termination()
