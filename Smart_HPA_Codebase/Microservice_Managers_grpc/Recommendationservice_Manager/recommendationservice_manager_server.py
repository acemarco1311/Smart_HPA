...
import recommendationservice_manager_pb2
import recommendationservice_manager_pb2_grpc
import subroutine
...
class RecommendationserviceManagerServicer(recommendationservice_manager_pb2_grpc.RecommendationserviceManagerServicer):
    def ExtractMicroserviceData(self, request, context):
        ...
        recommendationservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica, cpu_percentage]

        # example data
        # recommendationservice_data = ["recommendationservice", "no scale", 1, 1, 50, 5]
        # if error
        # recommendationservice_data = ["recommendationservice", "ERROR", 0, 0, 0, 0]

        return recommendationservice_manager_pb2.MicroserviceData(
            microservice_name = recommendationservice_data[0],
            scaling_action = recommendationservice_data[1],
            desired_replicas = recommendationservice_data[2],
            current_replicas = recommendationservice_data[3],
            cpu_request = recommendationservice_data[4],
            max_replicas = recommendationservice_data[5],
            cpu_percentage = recommendationservice_data[6]
        )

def serve(port):
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    recommendationservice_manager_pb2_grpc.add_RecommendationserviceManagerServicer_to_server(RecommendationserviceManagerServicer(), server)
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("recommendationservice Manager server started, listening on port " + port)
...
def serve_health(port):
    ...
    server.add_insecure_port("[::]:" + port)
    server.start()
    print("Heartbeat server for recommendationservice Manager started, listening on port " + port)
    server.wait_for_termination()
